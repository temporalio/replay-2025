  ...
  <div class="container space-y-8">
    ...
    <div class="grid grid-cols-1 gap-6 border border-grey bg-black p-6 lg:grid-cols-[auto_280px]">
      <div class="space-y-4 text-xl">
        <h3 class="font-afacad uppercase text-white">{speaker.jobTitle}</h3>
        <div class="prose prose-invert text-xl text-lilac">{@html biography}</div>
        {#if hasTalks}
          <div class="space-y-6 text-xl">
            <h3 class="font-afacad uppercase text-white">Talks</h3>
            <ul>
              {#each talks as talk}
                <li>
                  <a href="/schedule/{talk.fields.slug}" class="group space-y-2">
                    <h4 class="font-sans font-bold text-white group-hover:underline">
                      {talk.fields.title}
                    </h4>
                    <p class="font-sans text-lilac">{@html talk.date}</p>
                  </a>
                </li>
              {/each}
            </ul>
